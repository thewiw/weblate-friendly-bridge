...
// These tests assume the default WFB_OPENAPI state; another test file sets
...
vi.hoisted(() => {
  process.env.WFB_OPENAPI = '';
});

describe('REST API authentication', () => {
  it('hides the OpenAPI spec unless WFB_OPENAPI is enabled (default off)', async () => {
    const { app } = makeApp();
    const res = await request(app).get('/api/rest/v1/openapi.json').expect(404);
    expect((res.body as { error: string }).error).toContain('WFB_OPENAPI');
  });
