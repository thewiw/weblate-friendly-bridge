...
- Row selection: client-side `Selection { all, keys }` (keys = exceptions; selected when all=false, deselected when all=true) in `client/state/selection.ts`; cleared when the filtered set changes. Bulk tools (`POST /bulk-state`, polled via `/bulk-state/:jobId`) apply a translation state to every cell of the selected rows in the visible languages, patching Weblate with target+state (required) and updating the cache cell in place. **Search & replace** (`POST /search-replace/preview` synchronous cache scan + `POST /search-replace` background job, polled via the same `/bulk-state/:jobId`): same selection scope as bulk-state; literal-text search (regex-escaped, `$` in replace is inert), `ignoreCase`/`wholeWord` (whole word = unicode letter/digit lookarounds), per-visible-language choice; preview returns per-cell before/after; apply PATCHes each match keeping the state, updating cache cells in place; patches of the same string (row) are serialized — Weblate answers 500 when one string's source and translation units are patched concurrently; rate-budget checked like bulk-state. Source-language matches (opt-in in the dialog) match against `row.source` and update the row, not a cell.
