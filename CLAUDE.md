...
Configuration goes through compose environment variables (`WEBLATE_URL`, `WEBLATE_API_KEY`, `WEBLATE_REVIEW_WORKFLOW`, `MOCK_WEBLATE`, `WEBLATE_EXPORT_API_KEY` + `WEBLATE_EXPORT_ALLOWED_HOSTS` for public export, `SWAGGER_UI` to opt into the REST docs page) — no `.env` file is baked into the image. `WEBLATE_URL` must be reachable from inside the container: use `http://host.docker.internal:<port>` for a Weblate on the host's loopback (the compose file already adds the `host-gateway` mapping), or the LAN IP otherwise. Healthcheck probes `/api/v1/health` (the router is mounted under `/api/v1`, not `/api`).

Configuration lives in `.env` (see `.env.example`): `WEBLATE_URL`, optional `WEBLATE_API_KEY` (omitting it enables session mode: users sign in via the login view, backend holds their Weblate session server-side), `MOCK_WEBLATE=true` for fully offline dev (deterministic in-memory Weblate, also editable), `WEBLATE_EXPORT_API_KEY`/`WEBLATE_EXPORT_ALLOWED_HOSTS` for public (key-less) REST export, and `SWAGGER_UI=true|on|1` (read-only) or `SWAGGER_UI=try|with-try|with_try|swagger-with-try|swagger_with_try` (docs + "Try it out"; matching is case- and blank-insensitive) to serve the REST API docs.
...
- **Internal-host URLs**: Weblate advertises `units_list_url` and pagination `next` links on an internal hostname (e.g. 172.16.0.6) that may be unreachable. Always build URLs from the configured `WEBLATE_URL` base (`client.ts` helpers) and rewrite `next` links to the base origin (`paginate.ts`).
...
- Review workflow is detected per-project (`translation_review`); `WEBLATE_REVIEW_WORKFLOW` is only the fallback. "Not verified" filter = state 0/10, plus 20 (not approved) only when reviews are on.
